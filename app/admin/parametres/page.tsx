...
export default function ParametresPage() {
  ...
  async function handleTestToken() {
    if (!tmdbToken.trim()) { toast.error("Entrez un token TMDb"); return }
    setTesting(true)
    try {
      ...
      const json = await res.json()
      if (json.valid) { toast.success("Token TMDb valide !"); setTmdbStatus("valid") }
      else { toast.error("Token TMDb invalide"); setTmdbStatus("invalid") }
  ...
  async function handleSaveToken() {
    ...
    try {
      ...
      if (json.success) {
        ...
        setTmdbStatus("valid")
        setTmdbToken("")
      } else { toast.error(json.error || "Erreur lors de la sauvegarde") }
  ...
  return (
    <div>
      ...
      <Card className="mb-6">
        ...
        <CardContent className="flex flex-col gap-4">
          <p className="text-sm text-muted-foreground">
            Le token API TMDb est necessaire pour rechercher et recuperer les films. Obtenez-en
            un gratuitement sur{" "}
            <a
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={handleTestToken} disabled={testing || !tmdbToken.trim()}>
              {testing ? "Test en cours..." : "Tester le token"}
            </Button>
            <Button size="sm" onClick={handleSaveToken} disabled={saving || !tmdbToken.trim()}>
